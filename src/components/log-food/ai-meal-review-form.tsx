// components/log-food/ai-meal-review-form.tsx
...
interface ItemWithPortion extends AIMealItem {
  portionMultiplier: number; // 1.0 = 100%, 0.5 = 50%, 2.0 = 200%
}

interface AIMealReviewFormProps {
  title: string;
  onBack: () => void;
...
export function AIMealReviewForm({
  title,
  onBack,
  mealDescription,
  items,
  ...
  const adjustedTotals = useMemo(() => {
    return itemPortions.reduce(
      (acc, item) => ({
        calories: acc.calories + item.calories * item.portionMultiplier,
  ...
  const handleConfirmPress = () => {
    // Round the totals before confirming
    const roundedMacros = {
  ...
  return (
    ...
    >
      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
        ...
        {/* Custom content (assumptions, etc.) */}
        {children}
        ...
        <View style={styles.itemHeaderLeft}>
          ...
          <View style={styles.itemHeaderText}>
            ...
            <Text style={[styles.itemQuantity, { color: theme.icon }]}>
              {item.quantity} • {adjustedMacros.calories} cal
            </Text>
          </View>
        </View>
        <Ionicons
